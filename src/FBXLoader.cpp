...
std::shared_ptr<RiggedModel> FBXLoader::loadModel(const std::string& filePath)
{
    ...
    for (const auto& mesh : model->meshes)
    {
        for (const auto& meshBone : mesh.bones)
        {
            ...
            // FBX files can carry a skin bind that differs from the scene node bind pose.
            // Preserve the first imported skin bind per bone so we can correct rigs like
            // Worker.fbx at runtime without changing the normal path for well-behaved files.
